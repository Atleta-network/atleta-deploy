...
usage() {
    ...
    printf "\t<PREFIX><BABE, GRAN, IMON, PARA, ASGN, AUDI, BEEF>_<PRIVATE, PUBLIC>"
    printf "\033[0m\n"
}
...
load_envs() {
    ...
    local envs_postfix="BABE_PRIVATE BABE_PUBLIC GRAN_PRIVATE GRAN_PUBLIC IMON_PRIVATE IMON_PUBLIC PARA_PRIVATE PARA_PUBLIC ASGN_PRIVATE ASGN_PUBLIC AUDI_PRIVATE AUDI_PUBLIC BEEF_PRIVATE BEEF_PUBLIC"
    
    for postfix in $envs_postfix; do
        local variable_name="${prefix}${postfix}"

        # Use a fallback default value if the variable is not set
...
add_session_keys() {
    ...
    local para_private="${prefix}PARA_PRIVATE"
    local para_public="${prefix}PARA_PUBLIC"

    local asgn_private="${prefix}ASGN_PRIVATE"
    local asgn_public="${prefix}ASGN_PUBLIC"

    local audi_private="${prefix}AUDI_PRIVATE"
    local audi_public="${prefix}AUDI_PUBLIC"

    local beef_private="${prefix}BEEF_PRIVATE"
    local beef_public="${prefix}BEEF_PUBLIC"
    ...
    add_key "para" "${!para_private}" "${!para_public}" "$rpc_api_endpoint"
    add_key "asgn" "${!asgn_private}" "${!asgn_public}" "$rpc_api_endpoint"
    add_key "audi" "${!audi_private}" "${!audi_public}" "$rpc_api_endpoint"
    add_key "beef" "${!beef_private}" "${!beef_public}" "$rpc_api_endpoint"
}
